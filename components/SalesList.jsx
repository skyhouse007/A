...
const SalesList = ({ theme }) => {
  ...
  const fetchSales = async () => {
    try {
      ...
      setSales(salesArray);
      console.log("Fetched sales data:", salesArray);
    } catch (err) {
  ...
  const handleDownloadPDF = async (saleId) => {
    try {
      const pdfBlob = await salesService.getSalePDF(saleId);
      // For React Native, you would handle PDF differently
      // This is a placeholder for PDF handling
      Alert.alert('PDF', 'PDF download functionality needs to be implemented for mobile');
    } catch (err) {
  ...
  const handleDeleteAll = async () => {
    Alert.alert(
      'Delete All Sales',
      'Are you sure you want to delete ALL filtered sales? This cannot be undone.',
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Delete All',
          ...
          onPress: async () => {
            setDeleting(true);
            try {
              await Promise.all(filteredSales.map(sale => salesService.deleteSale(sale._id)));
              setSales((prev) => prev.filter((s) => !filteredSales.some(fs => fs._id === s._id)));
              setFilteredSales([]);
              Alert.alert('Success', 'All sales deleted successfully');
            } catch (err) {
              Alert.alert('Error', 'Failed to delete all sales');
              console.error(err);
            } finally {
              setDeleting(false);
            }
